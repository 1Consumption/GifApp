...
final class GifCellUseCaseTests: XCTestCase {

    private var gifCellUseCase: GifCellUseCase!
    ...
    func testRetrieveImageSuccess() {
        ...
        gifCellUseCase = GifCellUseCase(imageManager: imageManager, favoriteManager: favoriteManager)
        
        let _ = gifCellUseCase.retrieveImage(with: "test") { result in
            switch result {
            case .success:
    ...
    func testSendFavoriteStateChange() {
        ...
        gifCellUseCase.sendFavoriteStateChange(gifInfo: gifInfo) { result in
            switch result {
        ...
        DispatchQueue.global().asyncAfter(deadline: .now() + 1, execute: {
            self.gifCellUseCase.sendFavoriteStateChange(gifInfo: self.gifInfo) { result in
                switch result {
                case .success(let factor):
